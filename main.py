...
from models.model import (
    create_user, get_movie, statistika_user, statistika_movie,
    create_movie, get_channels, create_channel, delete_channel,
    get_users, delete_movie, get_channels_all
)
...
async def welcome_handler(msg: types.Message):
    ...
    await bot.set_my_commands([types.BotCommand("start", "Ishga tushirish ♻️")])
    await bot.send_message(
        msg.chat.id,
        text=f"Assalomu alaykum {msg.from_user.first_name} 🤖\n<b>Primiyera kino </b> orqali siz o'zingizga yoqqan kinoni topishingiz mumkin 🎬\nShunchaki kino kodini yuboring va kinoni oling ✅",
...
async def handle_video(msg: types.Message, state: FSMContext):
    if msg.text == "❌":
        ...
        if msg.video:
            ...
            await msg.answer("Iltimos Kino uchun ID kiriting: ", reply_markup=exit_btn())
        else:
...
async def add_channel_handler(msg: types.Message):
    if msg.from_user.id == int(ADMIN):
        ...
        await msg.answer("Qo'shish kerak bo'lgan kanal Usernameni kiriting ✍️", reply_markup=exit_btn())
    else:
        await msg.answer("Siz admin emassiz ❌", reply_markup=types.ReplyKeyboardRemove())
...
async def add_channel_username_handler(msg: types.Message, state: FSMContext):
    if msg.text == "❌":
        ...
        await msg.answer("Iltimos Kanal ID kiriting: ", reply_markup=exit_btn())
...
async def movie_delete_handler(msg: types.Message):
    if msg.from_user.id == int(ADMIN):
        ...
        await msg.answer("O'chirish kerak bo'lgan kanal ID kiriting ✍️", reply_markup=exit_btn())
    else:
        await msg.answer("Siz admin emassiz ❌", reply_markup=types.ReplyKeyboardRemove())
...
async def reklama_handler(msg: types.Message):
    if msg.from_user.id == int(ADMIN):
        ...
        await bot.send_message(msg.chat.id, "Reklama tarqatish bo'limi 🤖", reply_markup=exit_btn())
    else:
        await bot.send_message(msg.chat.id, "Siz admin emassiz ❌", reply_markup=types.ReplyKeyboardRemove())
...
async def rek_state(msg: types.Message, state: FSMContext):
    if msg.text == "❌":
        await bot.send_message(msg.chat.id, "Reklama yuborish bekor qilindi 🤖❌", reply_markup=admin_btn())
        await state.finish()
    else:
        await bot.send_message(msg.chat.id, "Reklama yuborish boshlandi 🤖✅", reply_markup=admin_btn())
        await state.finish()
        try:
            summa = 0
            for user in get_users():
                if int(user['telegram_id']) != int(ADMIN):
                    try:
                        ...
                        summa += 1
            await bot.send_message(int(ADMIN), f"Botni bloklagan Userlar soni: {summa}")
        except Exception as e:
            print(f"Error: {e}")
